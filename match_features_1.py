...
def match_from_paths(
    ...
    for idx, data in enumerate(tqdm(loader, smoothing=0.1)):
        ...
        # Run your RANSAC here. Adjust as needed:
        ...
        # Rebuild the matches0 and scores arrays after RANSAC filtering
        new_matches0 = -1 * np.ones_like(matches0)
        new_scores0 = np.zeros_like(mconf, shape=matches0.shape)  # same shape as matches0

        valid_ids = np.where(valid)[0]
        inlier_ids = valid_ids[inliers]

        # Set inlier matches and scores
        new_matches0[inlier_ids] = matches0[inlier_ids]
        # mconf now shorter, index with inliers:
        new_scores0[inlier_ids] = mconf
