...
class ImageDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # image_copy = read_image(self.root / name, False)
        image_copy = cv2.imread(self.root / name)
...
def main(
    ...
    yolo_model = YOLO("/home/luffy/continue/ultralytics/runs/detect/train/weights/best.pt")
    ...
    with h5py.File(str(feature_path), "a", libver="latest") as fd:
        for idx, data in enumerate(tqdm(loader)):
            ...
            yolo_pred = yolo_model(data["original"][0].numpy(), verbose=False)
            ...
            if "keypoints" in pred:
                ...
                if len(bboxes) > 0:
                    ...
                    valid_keypoints = keypoints[~inside_bbox]
                    valid_scores = np.array(pred["scores"])[~inside_bbox]
                    valid_descriptors = np.array(pred["descriptors"])[:,~inside_bbox]
                    
                    pred["keypoints"] = valid_keypoints
                    pred["scores"] = valid_scores
                    pred["descriptors"] = valid_descriptors
